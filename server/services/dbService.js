...
export const dbService = {
    ...
    async getStatistics(period) {
        ...
        const statistics = await StatisticsForDay.find({
            dayOfCapture: { $gte: moment(new Date()).subtract(period, 'days') },
        })
            .sort({ dayOfCapture: 1 })
            .exec();
        return statistics.map((statisticsMember) => {
            return {
                ...statisticsMember._doc,
                dayOfCapture: moment(statisticsMember.dayOfCapture).format('DD/MM/YYYY'),
            };
        });
    },
};
